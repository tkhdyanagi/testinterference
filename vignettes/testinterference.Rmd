...
The **testinterference** package provides tools to test several hypotheses about spillover effects.
...
- **Fisher's sharp null hypothesis** (**Fisher**):
...
In other words, we consider the case where SUTVA holds.

Run the `testinterference()` function:
...
In other words, we find no statistical evidence against SUTVA.
...
Here, `FALSE` indicates the acceptance of the null hypothesis.
Again, the results suggest that SUTVA might hold.
...
We turn to the case with pairwise interactions and complete randomization.
To allow for the presence of spillovers, we specify a positive value of `beta_s`.
...
```{r, eval = T, echo = T}
set.seed(1)
RT2 <- testinterference(Y                 = data2$Y,
                        Z                 = data2$Z,
                        A                 = data2$A,
                        hypothesis        = "SUTVA",
                        method            = "MIS",
...
All p-values are sufficiently small.
For the results of Simes' correction, `TRUE` indicates the rejection of the null hypothesis.
Thus, the testing results suggest the violation of SUTVA.
